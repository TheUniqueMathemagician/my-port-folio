	...
	return <div className={classNameBuilder.build()}>
		...
		<article className={classes["skills"]}>
			<Paper spaced className={classes["bubble"]} background="paper">
				...
				<Image alt="UI/UX" fill src="/images/solved.svg" />
			...
			<Paper spaced className={classes["bubble"]} background="paper">
				...
				<Image alt="UI/UX" fill src="/images/ui.svg" />
				<Typography variant="h6" tag="h5">UI / UX</Typography>
			</Paper>
			<Paper spaced className={classes["bubble"]} background="paper">
				{/* <div>Icons made by <a href="https://www.freepik.com" title="Freepik">Freepik</a> from <a href="https://www.flaticon.com/" title="Flaticon">www.flaticon.com</a></div> */}
				<Image alt="UI/UX" fill src="/images/innovation.svg" />
		...
		<article className={classes["education"]}>
				...
				{emEducation
					...
					.map((education) => (
						<li key={education.title}>
							<Typography variant="body" className={classes["date"]}>{education.date}</Typography>
							<Typography variant="h6">{education.title}</Typography>
							<Typography variant="body" color="hint">{education.detail}</Typography>
						</li>
					))}
			</ul>
			...
			<ul>
				{itEducation
					...
					.map((education) => (
						<li key={generateID()}>
							<Typography variant="body" className={classes["date"]}>/</Typography>
							<Typography variant="h6">{education.title}</Typography>
							<Typography variant="body" color="hint">{education.detail}</Typography>
						</li>
					))}
