...
const Settings: RunningApplicationComponent = (props) => {
	...
	const handleBackgroundInputChange = useCallback((event: ChangeEvent<HTMLInputElement>) => {
		setBackgroundColor({
			[ColorScheme.contrast]: event.target.value,
			[ColorScheme.dark]: event.target.value,
			[ColorScheme.default]: event.target.value,
			[ColorScheme.light]: event.target.value,
		})
	}, [setBackgroundColor])
	...
	const handlePrimaryInputChange = useCallback((event: ChangeEvent<HTMLInputElement>) => {
		setPrimaryColor({
			[ColorScheme.contrast]: event.target.value,
			[ColorScheme.dark]: event.target.value,
			[ColorScheme.default]: event.target.value,
